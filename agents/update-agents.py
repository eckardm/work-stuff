...
for filename in os.listdir(path):
	...
	for agent in agents:
		if agent.tag == 'persname':
			if '--' in agent.text:
				agent.attrib['ref'] = persnames_dic[agent.text.split('--')[0].strip().rstrip('.')]
			else:
				agent.attrib['ref'] = persnames_dic[agent.text.strip().rstrip('.')]
		elif agent.tag == 'corpname':
			if '--' in agent.text:
				agent.attrib['ref'] = corpnames_dic[agent.text.split('--')[0].strip().rstrip('.')]
			else:
				agent.attrib['ref'] = corpnames_dic[agent.text.strip().rstrip('.')]
		elif agent.tag == 'famname':
			if '--' in agent.text:
				agent.attrib['ref'] = famnames_dic[agent.text.split('--')[0].strip().rstrip('.')]
			else:
				agent.attrib['ref'] = famnames_dic[agent.text.strip().rstrip('.')]
